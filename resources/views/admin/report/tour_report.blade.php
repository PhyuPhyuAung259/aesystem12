        ...
        <section class="col-lg-12 connetedSortable">
          ...
          <form target="_blank" method="POST" action="{{route('searchReport')}}">
            ...
            <div class="col-sm-8 pull-right" style="margin-bottom:10px;">
             ...
             <div class="col-md-2" id="flights" style="padding-right: 0px; display:none;">
              <select class="col-md-2 form-control input-sm  " name="flight" id="flight"  >
...
<script type="text/javascript">
   ...
   var flightElement=document.getElementById('flights');
   ...
   $('#country').change(function() {
    ...
    $.ajax({
        ...
        error: function(xhr, status, error) {
            console.error("Hotel AJAX Error:", status, error);
        }
    });

    // Flights AJAX
    $.ajax({
        url: '/flights/' + countryId,
        type: 'GET',
        dataType: 'json',
        success: function(response) {
            var flightSelect = $('#flight');
            flightSelect.empty();
            if (response.length === 0) {
                flightSelect.append('<option value="0">No flights available</option>');
            } else {
                flightSelect.append('<option value="0">Choose Flight</option>');
                $.each(response, function(key, value) {
                    flightSelect.append('<option value="' + value.id + '">' +
                        value.supplier_name + '</option>');
                });
            }
        },
        error: function(xhr, status, error) {
            console.error("Flight AJAX Error:", status, error);
        }
    });
});
