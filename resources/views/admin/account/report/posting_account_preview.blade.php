...
@section('title',$project['project_number']." - Preview Post Account")
                    ...
                    <table class="table table-bordered">
                        <?php 
                            ...
                            if (empty((int)$project->project_selling_rate)) {
                                ...
                                $Project_total = ($project->project_selling_rate + $project->project_add_invoice) - $project->project_cnote_invoice;
                            }
                            $totalRevenue = ($Project_total+$project->project_nadd_invoice) - $project->project_ncnote_invoice; 
                       ...
                       @if(!empty($Project_total))
                            ...
                            <tr>
                                ...
                                <td class="" colspan="2">
                                        <?php
                                            ...
                                            $proJournal = App\AccountJournal::where([
                                            ...
                                            ->where('account_name_id', '!=', 333)
                                            ->where('account_type_id', '!=', 10)
                                            ->first();
                                            $proRefundJournal = App\AccountJournal::where([
                                                'supplier_id'    => $project->supplier_id,
                                                'business_id'    => 9,
                                                'project_number' => $project->project_number,
                                                'book_id'        => $project->id,
                                            ...
                                            if(isset($proJournal)){
                                                ...
                                                $accTransaction = App\AccountTransaction::where(['journal_id'=> $proJournal->id, 'account_type_id'=>8, 'status'=>1])
                                                    ->where('account_name_id', '!=', 333);
                                                $color = $accTransaction->sum('debit') == $Project_total  ? "style=font-weight:700;color:#3c8dbc" : '';
                                            
                                            }else{
                                                $color=0;
                                                $accTransaction='';
                                            }
                                            if(isset($proRefundJournal)){
                                                $refundser=App\User::find($proRefundJournal->user_id);
                                                $accTransaction = App\AccountTransaction::where(['journal_id'=> $proRefundJournal->id, 'account_type_id'=>10, 'status'=>1]);
                                               //color = $accTransaction->sum('debit') == $Project_total  ? "style=font-weight:700;color:#3c8dbc" : '';
                                                $totalRevenue = $totalRevenue - $proRefundJournal->book_amount;
                                            }
                                            ?>
                                        @if(!empty($accTransaction) && $accTransaction->sum('debit') > 0)
                                            <span class="pull-left" {{$color}}> {{ $Project_total  == $accTransaction->sum('debit') ? Content::money($Project_total ) : Content::money($Project_total  - $accTransaction->sum('debit')) }}</span>
                                        @else
                                            {{Content::money($Project_total)}}
                                        @endif
                                    ...
                                    @if(isset($proJournal['book_amount']) && $proJournal['book_amount'] > 0 )	
                                            ...
                                            @if ($proJournal['book_amount'] > $accTransaction->sum('debit'))
                                                <a target="_blank" href="{{route('getPayable', ['journal_id'=> $proJournal->id])}}" class="btn btn-info btn-xs hidden-print pull-right"><b>Receive </b></a>                                            <span class="btn btn-link btn-xs hidden-print pull-right">
                                                    ...
                                                    <span data-toggle="modal" data-target="#loadProjectConfirm" class="btn btn-success btn-xs AddToACcount hidden-print pull-right"
                                                        ...
                                                        data-country="{{ $project->country_id}}">Refund</span>                                               
                                            @elseif( $Project_total  >= $proJournal['book_amount'])
                                        ...
                                        @else
                                            <span class="badge badge-light pull-right">{{ Content::money($accTransaction->sum('debit')) }}</span>
                                            @if ($proJournal['book_amount'] > $accTransaction->sum('debit'))
                                                <a target="_blank" href="{{route('getPayable', ['journal_id'=> $proJournal->id])}}" class="btn btn-info btn-xs hidden-print pull-right" readonly><b>Receive </b></a>                                            <span class="btn btn-link btn-xs hidden-print pull-right">
                                                        <a target="_blank" href="{{route('getJournalReport', ['journal_entry' => $project->project_number])}}" readonly>View/Edit</a>
                                                    </span>  
                                                    <span data-toggle="modal" data-target="#loadProjectConfirm" class="btn btn-success btn-xs AddToACcount hidden-print pull-right"
                                                        data-type="9" data-kamount="" 
                                                        data-amount="{{$Project_total}}" 
                                ...
                                @if(isset($proRefundJournal))
                                    <tr style="font-weight:700;color:red"> 
                                        <td >Refund Date : {{$proRefundJournal->entry_date}}</td>
                                        <td  colspan="9">{{$proRefundJournal->remark}}</td>
                                        <td >{{Content::money($proRefundJournal['book_amount']) }}</td>
                                        @if(\Auth::user()->role_id == 2)
                                                    <td>{{{$refundser->name or ''}}}</td>                                
                                                @endif
                                    </tr> 
                                @endif 
                        @endif
                        <!-- Additional Invoice -->
                        @if(!empty($project->project_nadd_invoice) && $project->project_nadd_invoice > 0)
                            <tr style="background-color:#f4f4f4;">
                                <th style="border-top: none;" width="170px">Travelling Date</th>
                                <th style="border-top: none;" colspan="8">Descriptions</th>
                                <th style="border-top: none; width: 15%;" colspan="2" class="text-left">INV-Amount <span class="pull-right">Received</span></th>
                            ...
                            <tr>
                                <td>{{ Content::dateformat($project->project_start) }} -> {{ Content::dateformat($project->project_end) }}</td>
                                <td colspan="8">{!! $project->project_add_desc !!}
                                
                                </td>
                                <td class="" colspan="2">
                                        <?php
                                            $proJournal = App\AccountJournal::where(['supplier_id'=>$project->supplier_id, 'business_id'=>9,'project_number'=>$project->project_number, "book_id"=>$project->id, 'status'=>1,'account_name_id'=>333])->first();
                                            
                                            if(isset($proJournal)){
                                                $user=App\User::find($proJournal->user_id);
                                                $accTransaction = App\AccountTransaction::where(['journal_id'=> $proJournal->id, 'account_type_id'=>8, 'status'=>1]);
                                                $color = $accTransaction->sum('debit') == $project->project_nadd_invoice  ? "style=font-weight:700;color:#3c8dbc" : '';
                                            }else{
                                                $color=0;
                                                $accTransaction='';
                                            }
                                        ...
                                        @if(!empty($accTransaction))
                                            <span class="pull-left" {{$color}}> {{ $project->project_nadd_invoice  == $accTransaction->sum('debit') ? Content::money($project->project_nadd_invoice ) : Content::money($project->project_nadd_invoice  - $accTransaction->sum('debit')) }}</span>
                                        @else
                                            {{Content::money($project->project_nadd_invoice)}}
                                        @endif
                                    ...
                                    @if(isset($proJournal['book_amount']) && $proJournal['book_amount'] > 0)	
                                        @if(\Auth::user()->role_id == 2 || \Auth::user()->role_id == 3)
                                            ...
                                            @elseif( $project->project_nadd_invoice  >= $proJournal['book_amount'])
                                                <span class="btn btn-link btn-xs hidden-print pull-right">
                                                    <a target="_blank" href="{{route('getJournalReport', ['journal_entry' => $project->project_number])}}">View/Edit</a>
                                                </span>
                                            @endif
                                    ...
                                    @else
                                        @if($project->project_nadd_invoice  > 0)
                                            <span data-toggle="modal" data-target="#loadProjectConfirm" class="btn btn-success btn-xs AddToACcount hidden-print pull-right"
                                                data-type="9" data-kamount="" 
                                                data-amount="{{$project->project_nadd_invoice}}" 
                                                data-process_type="receive" 
                                                data-bus_type="agent"
                                                data-title="{{{ $project->supplier->supplier_name or ''}}}"
                                                data-book_id="{{$project->id}}"
                        ...
                        @endif
                        <!-- Additional Invoice end -->
                          <!-- Credit Note -->
                        @if(!empty($project->project_ncnote_invoice))
                            
                            <tr style="background-color:#f4f4f4;">
                                <th style="border-top: none;" width="170px">Travelling Date</th>
                                <th style="border-top: none;" colspan="8">Descriptions</th>
                                <th style="border-top: none; width: 15%;" colspan="2" class="text-left">INV-Amount <span class="pull-right">Paid</span></th>
                                @if(\Auth::user()->role_id == 2)
                                    <th style="border-top: none;">Posted by</th>
                                @endif
                            ...
                            <tr>
                                <td>{{ Content::dateformat($project->project_start) }} -> {{ Content::dateformat($project->project_end) }}</td>
                                <td colspan="8">{!! $project->project_note_desc !!}
                                
                                </td>
                                <td class="" colspan="2">
                                        <?php
                                            $proJournal = App\AccountJournal::where(['supplier_id'=>$project->supplier_id, 'business_id'=>9,'project_number'=>$project->project_number, "book_id"=>$project->id, 'account_type_id'=>10, 'status'=>1])->first();
                                            
                                            if(isset($proJournal)){
                                                $user=App\User::find($proJournal->user_id);
                                                $accTransaction = App\AccountTransaction::where(['journal_id'=> $proJournal->id, 'account_type_id'=>10, 'status'=>1]);
                                                $color = $accTransaction->sum('debit') == $project->project_ncnote_invoice  ? "style=font-weight:700;color:#3c8dbc" : '';
                                            }else{
                                                $color=0;
                                                $accTransaction='';
                                            }
                                            ?>
                                        @if(!empty($accTransaction))
                                            <span class="pull-left" {{$color}}> {{ $project->project_ncnote_invoice  == $accTransaction->sum('credit') ? Content::money($project->project_ncnote_invoice ) : Content::money($project->project_ncnote_invoice  - $accTransaction->sum('credit')) }}</span>
                                        @else
                                            {{Content::money($project->project_ncnote_invoice)}}
                                        @endif

                                    @if(isset($proJournal['book_amount']) && $proJournal['book_amount'] > 0 )	
                                        @if(\Auth::user()->role_id == 2 || \Auth::user()->role_id == 3)
                                       
                                            <span class="badge badge-light pull-right">{{ Content::money($accTransaction->sum('credit')) }}  </span>
                                            @if ($proJournal['book_amount'] > $accTransaction->sum('credit'))
                                                <a target="_blank" href="{{route('getPayable', ['journal_id'=> $proJournal->id])}}" class="btn btn-info btn-xs hidden-print pull-right"><b>Pay </b></a>                                            <span class="btn btn-link btn-xs hidden-print pull-right">
                                                        <a target="_blank" href="{{route('getJournalReport', ['journal_entry' => $project->project_number])}}">View/Edit</a>
                                                    </span>   
                                            @elseif( $project->project_ncnote_invoice  >= $proJournal['book_amount'])
                                                <span class="btn btn-link btn-xs hidden-print pull-right">
                                                    <a target="_blank" href="{{route('getJournalReport', ['journal_entry' => $project->project_number])}}">View/Edit</a>
                                                </span>
                                            @endif
                                    ...
                                    @else
                                        @if($project->project_ncnote_invoice  > 0)
                                            <span data-toggle="modal" data-target="#loadProjectConfirm" class="btn btn-success btn-xs AddToACcount hidden-print pull-right"
                                                data-type="9" data-kamount="" 
                                                data-amount="{{$project->project_ncnote_invoice}}" 
                                                data-process_type="pay" 
                                                data-bus_type="agent"
                                                data-title="{{{ $project->supplier->supplier_name or ''}}}"
                                                data-book_id="{{$project->id}}"
                                ...
                                <td class="text-center" colspan="2">
                                    ...
                                    @if( $proJournal['book_amount'] > 0 ) 
                                        ...
                                        @else
                                            ...
                                            @if ($proJournal['book_amount'] > $accTransaction->sum('credit'))
                                                <a target="_blank" href="{{route('getPayable', ['journal_id'=> $proJournal->id])}}" class="btn btn-info btn-xs hidden-print pull-right" readonly><b>Receive</b></a>
                        ...
                        @if($hotelBook->count()>0)
                            ...
                            @foreach($hotelBook as $hotel)
                                <tr class="container_hotel">
                                    ...
                                    <td class="text-right" width="120px">
                                        <?php 
                                            ...
                                            $hrefundJournal = App\AccountJournal::where(['supplier_id'=>$hotel->hotel->id, 'business_id'=>1,'project_number'=>$project->project_number, "book_id"=>$hotel->id, 'status'=>1, 'account_type_id' => 8])->first();
                                        //  dd($hrefundJournal);
                                            ...
                                            if(isset($hrefundJournal)){
                                                $hrefundTransaction = App\AccountTransaction::where(['journal_id'=>$hrefundJournal['id'], 'status'=>1]); 
                                              //  $color = $hTransaction->sum('debit') == $hotelAmount ? "style=font-weight:700;color:#3c8dbc" : '';	
                                                $hrefunduser=App\User::find($hrefundJournal->user_id);
                                                $totalHotel = $totalHotel - $hrefundJournal['book_amount'];
                                            }                         
                                        ?>
                                        ...
                                        @if(isset($HproJournal) && !empty($HproJournal['book_amount']))
                                            ...
                                            @if ( $HproJournal['book_amount'] > $hTransaction->sum('credit'))
                                                ...
                                                <span data-toggle="modal" data-target="#loadProjectConfirm" class="btn btn-success btn-xs AddToACcount text-right hidden-print pull-right"
                                                    ...
                                                    data-country="{{{ $hotel->hotel->country_id or ''}}}">Return
                                                </span>
                                ...
                                @if(isset($hrefundJournal))
                                <tr style="font-weight:700;color:red"> 
                                    <td >Refund Date : {{$hrefundJournal->entry_date}}</td>
                                    <td  colspan="9">{{$hrefundJournal->remark}}</td>
                                    <td >{{Content::money($hrefundJournal['book_amount']) }}</td>
                                    @if(\Auth::user()->role_id == 2)
                                                <td>{{{$hrefunduser->name or ''}}}</td>                                
                                            @endif
                                </tr> 
                                @endif 
                            @endforeach
                                ...
                                <tr>
                                    <td colspan="12" class="text-right"><h5><strong>Total: {{ Content::money($totalHotel)}} {{Content::currency()}} </strong></h5></td>
                                </tr>
                        @endif
                            ...
                            @foreach($flightBook as $fl)
                                    ...
                                    <tr class="container_flight">
                                        ...
                                        <?php
                                            ...
                                            $flightAmount = $fl->book_namount + $fl->book_nkamount + $flightAmount;
                                            ...
                                            $FrefundJournal = App\AccountJournal::where(['supplier_id'=>$flprice['id'], 'business_id'=>4,'project_number'=>$project->project_number, "book_id"=>$fl->id, 'type'=>1, 'account_type_id' => 8, 'status'=>1])->first();
                                                if(isset($FJournal)){
                                                ...
                                                if(isset($FrefundJournal)){
                                                    $frefunduser=App\User::find($FrefundJournal->user_id);
                                                    $frefundSaction = App\AccountTransaction::where(['journal_id'=>$FrefundJournal['id'], 'status'=>1]);
                                                    $flightAmount= $flightAmount - $FrefundJournal['book_amount'];
                                                }									
                                        ?>
                                        ...
                                        <td class="text-right">
                                            ...
                                            @endif
                                            @if(isset($FJournal) && !empty($FJournal['book_amount']))
                                            @if($FJournal['book_amount'] > 0 || $FJournal['book_kamount'] > 0 )
                                                ...
                                                @else
                                                  <span class="badge badge-light pull-right">{{ $fSaction->sum('credit') ? Content::money($fSaction->sum('credit')) : Content::money($fSaction->sum('kcredit')) }}</span>
                                                    @if($FJournal['book_amount'] > $fSaction->sum('credit') || $FJournal['book_kamount'] > $fSaction->sum('kcredit'))
                                                        <a target="_blank" href="{{route('getPayable', ['journal_id'=>$FJournal['id']]) }}" class="btn btn-info btn-xs pull-right hidden-print" readonly><b>Pay</b></a>
                                                            <span class=" btn btn-link btn-xs hidden-print pull-right">
                                                        <a target="_blank" href="{{route('getJournalReport', ['journal_entry' => $project['project_number']])}}" readonly>View/Edit</a></span>
                                                        <span data-toggle="modal" data-target="#loadProjectConfirm" class="btn btn-success btn-xs AddToACcount text-right hidden-print pull-right" 
                                                                data-type="4" data-kamount="{{$fl->book_nkamount}}" 
                                                                data-amount="{{$fl->book_namount}}" 
                                                                data-process_type="recieve" 
                                    ...
                                    @if(isset($FrefundJournal))
                                    <tr style="font-weight:700;color:red">
                                        <td >Refund Date : {{$FrefundJournal->entry_date}}</td>
                                        <td  colspan="9">{{$FrefundJournal->remark}}</td>
                                        <td >{{Content::money($FrefundJournal['book_amount']) }}</td>
                                        @if(\Auth::user()->role_id == 2)
                                                    <td>{{{$frefunduser->name or ''}}}</td>                                
                                                @endif
                                    </tr> 
                                @endif 
                            @endforeach	
                            ...
                            <tr>
                                <td colspan="11" class="text-right">
                                    <h5><strong>
                                            ...
                                            @endif -->
                                            Total {{Content::currency()}}: {{ Content::money($flightAmount) or '' }}	
                                        </strong>
                                    </h5>
                                </td>
                            </tr>
                            ...
                            <tr style="background-color:#f4f4f4;">
                                ...
                                @foreach($golfBook as $gf)			
                                    <?php 
                                        ...
                                        $GrefundJournal = App\AccountJournal::where(['business_id'=>29, 'supplier_id'=>$gf->golf_id, 'project_number'=>$project['project_number'], "book_id"=>$gf->id, 'type'=>1, 'status'=>1, 'account_type_id' => 8])->first();
                                        if(isset($GJournal)){
                                        ...
                                        if(isset($GrefundJournal)){
                                            $grefunduser=App\User::find($GrefundJournal->user_id);
                                            $Grefundsaction = App\AccountTransaction::where(['journal_id'=>$GrefundJournal['id'], 'status'=>1]);
                                            // $color 	= $Gsaction->sum('credit') == $gf->book_namount ? "style=font-weight:700;color:#3c8dbc" : '';
                                            // $colork = $Gsaction->sum('kdebit') == $gf->book_nkamount ? "style=font-weight:700;color:#3c8dbc" : '';
                                            $TotalGolf = $TotalGolf - $GrefundJournal['book_amount'];
                                        }                                           
                                        ...
                                        @if(isset($GJournal['book_amount']) && $GJournal['book_amount'] > 0 || isset($GJournal['book_kamount']) && $GJournal['book_kamount'] > 0)
                                             @if(\Auth::user()->role_id == 2 || \Auth::user()->role_id == 3)
                                                    ...
                                                    @if($GJournal['book_amount'] > $Gsaction->sum('credit') || $GJournal['book_kamount'] > $Gsaction->sum('kcredit'))
                                                        ...
                                                        <span data-toggle="modal" data-target="#loadProjectConfirm" class="btn btn-success btn-xs AddToACcount text-right hidden-print pull-right"
                                                            ...
                                                            data-country="{{{ $gf->country_id or ''}}}">Return</span>
                                             ...
                                             @else
                                                 <span class="badge badge-light pull-right">{{ Content::money($Gsaction->sum('credit')) }}</span>
                                                    ...
                                                    @if($GJournal['book_amount'] > $Gsaction->sum('credit') || $GJournal['book_kamount'] > $Gsaction->sum('kcredit'))
                                                        <a target="_blank" href="{{route('getPayable', ['journal_id'=>$GJournal['id'] ])}}" class="btn btn-info btn-xs pull-right hidden-print" readonly><b>Pay</b></a>
                                    ...
                                    @if(isset($GrefundJournal))
                                        <tr style="font-weight:700;color:red">
                                            <td >Refund Date : {{$GrefundJournal->entry_date}}</td>
                                            <td  colspan="9">{{$GrefundJournal->remark}}</td>
                                            <td >{{Content::money($GrefundJournal['book_amount']) }}</td>
                                            @if(\Auth::user()->role_id == 2)
                                                        <td>{{{$grefunduser->name or ''}}}</td>                                
                                                    @endif
                                        </tr> 
                                    @endif 
                                @endforeach
                        ...
                        @if($cruiseBook->count() > 0)
                            ...
                            @foreach($cruiseBook as $crp)			
                                <?php 
                                    ...
                                    $RrefundJournal = App\AccountJournal::where(['book_id'=>$crp->id, 'supplier_id'=>$crp->cruise_id, 'business_id'=>3,'project_number'=>$project->project_number, 'status'=>1, 'type'=>1, 'account_type_id' => 8 ])->first();
                                    if(isset($RJournal)){
                                    ...
                                    if(isset($RrefundJournal)){
                                        $rrefunduser=App\User::find($RrefundJournal->user_id);
                                        $rrefundsaction = App\AccountTransaction::where(['journal_id'=>$RrefundJournal['id'], 'status'=>1]); 
                                        //$color 	= $rrefundsaction->sum('credit') == $crp->net_amount ? "style=font-weight:700;color:#3c8dbc" : '';
                                        $totalCruise = $totalCruise - $RrefundJournal['book_amount'];
                                    }                                 
                                ?>	
                                    ...
                                    <td class="text-right">	
                                        ...
                                        @if(isset($RJournal['book_amount']) || $RJournal['book_kamount'] > 0 )
                                            @if(\Auth::user()->role_id == 2 || \Auth::user()->role_id == 3)
                                                ...
                                                @if($RJournal['book_amount'] > $rsaction->sum('credit') || $RJournal['book_kamount'] > $rsaction->sum('kcredit'))
                                                    ...
                                                    <span data-toggle="modal" data-target="#loadProjectConfirm" class="btn btn-success btn-xs AddToACcount text-right hidden-print pull-right" 
                                                        ...
                                                        data-country="{{$crp->cruise->country_id}}">Return</span>
                                            ...
                                            @else
                                             <span class="badge badge-light pull-right">{{{ Content::money($rsaction->sum('credit')) or '' }}}</span>
                                                @if($RJournal['book_amount'] > $rsaction->sum('credit') || $RJournal['book_kamount'] > $rsaction->sum('kcredit'))
                                                    <a target="_blank" href="{{route('getPayable', ['journal_id'=>$RJournal['id']])}}" class="btn btn-info  btn-xs pull-right hidden-print" readonly><b>Pay</b></a>
                                                    <span class=" btn btn-link btn-xs hidden-print pull-right">
                                                        <a  target="_blank" href="{{route('getJournalReport', ['journal_entry' => $project->project_number])}}" readonly>View/Edit</a>
                                                    </span>
                                                    <span data-toggle="modal" data-target="#loadProjectConfirm" class="btn btn-success btn-xs AddToACcount text-right hidden-print pull-right" 
                                ...
                                @if(isset($RrefundJournal))
                                <tr style="font-weight:700;color:red">
                                    <td >Refund Date : {{$RrefundJournal->entry_date}}</td>
                                    <td  colspan="9">{{$RrefundJournal->remark}}</td>
                                    <td >{{Content::money($RrefundJournal['book_amount']) }}</td>
                                    @if(\Auth::user()->role_id == 2)
                                                <td>{{{$rrefunduser->name or ''}}}</td>                                
                                            @endif
                                </tr> 
                            @endif 
                            @endforeach
                        ...
                        @if( $tranBook->count() != 0)
                            ...
                            <?php 
                                ...
                                if ($btran !== null && is_object($btran)) {
                                    ...
                                    $TranRefundJournal = App\AccountJournal::where(['supplier_id'=>$btran['transport_id'], 'business_id'=>7,'project_number'=>$project->project_number, 'book_id'=>$tran->id, 'status'=>1,'account_type_id' => 8])->first();
                                    ...
                                    if(isset($TranRefundJournal)){
                                        $tranrefunduser=App\User::find($TranRefundJournal->user_id);
                                        $TransrefundacTran = App\AccountTransaction::where(['journal_id'=>$TranRefundJournal['id'], 'status'=>1]);
                                        // $color 	= $TransacTran->sum('credit') == $btran['price'] ? "style=font-weight:700;color:#3c8dbc" : '';
                                        // $colork	= $TransacTran->sum('kcredit') == $btran['kprice'] ? "style=font-weight:700;color:#3c8dbc" : '';
                                        $transportTotal = $transportTotal - $TranRefundJournal['book_amount'];
                                    }  
                                }                                            
                            ?>
                                        ...
                                        @if(isset($TranJournal['book_amount']) || isset($TranJournal['book_kamount']))
                                            ...
                                            @if($TranJournal['book_amount'] > 0 || $TranJournal['book_kamount'] > 0)
                                                @if(\Auth::user()->role_id == 2 || \Auth::user()->role_id == 3)
                                                    ...
                                                    @if($TranJournal['book_amount'] > $TransacTran->sum('credit') || $TranJournal['book_kamount'] > $TransacTran->sum('kcredit'))
                                                        ...
                                                        <span data-toggle="modal" data-target="#loadProjectConfirm" class="btn btn-success btn-xs AddToACcount text-right hidden-print pull-right" 
                                                            ...
                                                            data-country="{{$btran['country_id']}}">Return</span>
                                                ...
                                                @else
                                                    <span class="badge badge-light pull-right">{{ Content::money($TransacTran->sum('credit')) }}</span>
                                                    @if($TranJournal['book_amount'] > $TransacTran->sum('credit') || $TranJournal['book_kamount'] > $TransacTran->sum('kcredit'))
                                                        <a target="_blank" href="{{route('getPayable', ['journal_id'=>$TranJournal['id']])}}" class="btn btn-info btn-xs pull-right hidden-print" readonly><b>Pay</b></a>
                            ...
                            @if(isset($TranRefundJournal))
                                ...
                                <tr style="font-weight:700;color:red">
                                    <td >Refund Date : {{$TranRefundJournal->entry_date}}</td>
                                    <td  colspan="8">{{$TranRefundJournal->remark}}</td>
                                    <td  colspan="2" class="text-left">{{Content::money($TranRefundJournal['book_amount']) }}</td>
                                    @if(\Auth::user()->role_id == 2)
                                                <td  >{{{$tranrefunduser->name or ''}}}</td>                                
                                    @endif
                                </tr> 
                            @endif 				
                            @endforeach
                            ...
                            @foreach($guideBook as $tran)
                                <?php 
                                    ...
                                    if ($bg !== null && is_object($bg)) {
                                        ...
                                        $GrefundJournal=App\AccountJournal::where(['supplier_id'=>$bg['supplier_id'], 'business_id'=>6,'project_number'=>$project->project_number, 'book_id'=>$tran->id, 'type'=>2 ,'status'=>1, 'account_type_id' => 8])->first();
                                        if(isset($GJournal)){
                                        ...
                                        if(isset($GrefundJournal)){
                                            $guiderefundUser=App\User::find($GrefundJournal->user_id);
                                            $GrefundAccTran = App\AccountTransaction::where(['journal_id'=>$GrefundJournal['id'], 'status'=>1]);
                                            // $color 	= $GAccTran->sum('credit') == $bg['price'] ? "style=font-weight:700;color:#3c8dbc" : '';
                                            // $colork	= $GAccTran->sum('kcredit') == $bg['kprice'] ? "style=font-weight:700;color:#3c8dbc" : '';
                                            $guidTotal = $guidTotal - $GrefundJournal['book_amount'];
                                        }
                                    }
                                ?>
                                                 ...
                                                 @if(\Auth::user()->role_id == 2 || \Auth::user()->role_id == 3)
                                                    ...
                                                    @if($GJournal['book_amount'] > $GAccTran->sum('credit') || $GJournal['book_kamount'] > $GAccTran->sum('kcredit'))
                                                        ...
                                                        <span data-toggle="modal" data-target="#loadProjectConfirm" class="btn btn-success btn-xs AddToACcount text-right hidden-print pull-right" 
                                                            ...
                                                            data-country="{{$bg->country_id}}">Return</span>
                                ...
                                <tr style="font-weight:700;color:red">
                                    <td >Refund Date : {{$GrefundJournal->entry_date}}</td>
                                    <td  colspan="8">{{$GrefundJournal->remark}}</td>
                                    <td   colspan="2" >{{Content::money($GrefundJournal['book_amount']) }}</td>
                                    @if(\Auth::user()->role_id == 2)
                                                <td>{{{$guiderefundUser->name or ''}}}</td>                                
                                            @endif
                                </tr> 
                            @endif 				
                            @endforeach
                        ...
                        @if($restBook->count() > 0)
							...
							<?php 
                                $restaurant = App\BookRestaurant::where(['project_number'=>$rest->project_number, 'book_id'=>$rest->id])->first();
                              
                                $restTotal = $restTotal + $rest['amount'];
                                ...
                                $RestrefundJournal = App\AccountJournal::where(['supplier_id'=>$rest['supplier_id'], 'business_id'=>2,'project_number'=>$project->project_number, 'book_id'=>$rest->id, 'type'=>2, 'status'=>1 , 'account_type_id' => 8])->first();
                                    // dd($RestrefundJournal);
                                if(isset($RJournal)){
                                    $Ruser=App\User::find($RJournal->user_id);
                                ...
                                if(isset($RestrefundJournal)){
                                    $Rrefunduser=App\User::find($RestrefundJournal->user_id);
									$RestrefundAccTran = App\AccountTransaction::where(['journal_id'=>$RestrefundJournal['id'], 'status'=>1]);
									// $color 	= $RestAccTran->sum('credit') == $rest['amount'] ? "style=font-weight:700;color:#3c8dbc" : '';
									// $colork	= $RestAccTran->sum('kcredit') == $rest['kamount'] ? "style=font-weight:700;color:#3c8dbc" : '';
                                   $restTotal = $restTotal - $RestrefundJournal['book_amount'];
							...
							<tr class="container_restaurant">
				                ...
				                <td class="text-right">{{Content::money($rest->kprice)}}</td>
			                  	<td class="text-left">
									...
									@endif
			                  		@if(isset($RJournal['book_amount']) || isset($RJournal['book_kamount']))
    			                  		     ...
    			                  		     @if(\Auth::user()->role_id == 2 || \Auth::user()->role_id == 3)
        			                  			...
        			                  			@if($RJournal['book_amount'] > $RestAccTran->sum('credit') || $RJournal['book_kamount'] > $RestAccTran->sum('kcredit') )
        			  								...
        			  								<span data-toggle="modal" data-target="#loadProjectConfirm" class="btn btn-success btn-xs AddToACcount text-right hidden-print pull-right"
                                                        ...
                                                        data-country="{{$rest->country_id}}">Return</span>
                            ...
                            @if(isset($RestrefundJournal))
                                <tr style="font-weight:700;color:red">
                                    <td >Refund Date : {{$RestrefundJournal->entry_date}}</td>
                                    <td  colspan="7">{{$RestrefundJournal->remark}}</td>
                                    <td  colspan="3">{{Content::money($RestrefundJournal['book_amount']) }}</td>
                                    @if(\Auth::user()->role_id == 2)
                                                <td >{{{$Rrefunduser->name or ''}}}</td>                                
                                            @endif
                                </tr> 
								...
								<td colspan="11" class="text-right">
									<h5>
										<strong>
                                            ...
                                            Total {{Content::currency()}}: {{ Content::money($restTotal) }}

                                                <!-- @if(isset($RestrefundJournal))
                                                    $totalrest= $restBook->sum('amount') - $RestrefundJournal['book_amount']; 
                                                    Total {{Content::currency()}}: {{ Content::money($restTotal) }}
                                                @else
                                                    Total {{Content::currency()}}: {{ Content::money($restBook->sum('amount')) }}
                                                @endif
												<!-- Total {{Content::currency()}}: {{ Content::money($restBook->sum('amount') - $RestrefundJournal['book_amount']) }} -->
											@endif
						...
						@if($EntranceBook->count() > 0)
							...
							@foreach($EntranceBook as $ent)
								<?php 
                                    ...
                                    $EntrefundJournal = App\AccountJournal::where(['business_id'=>55,'project_number'=>$project->project_number, 'book_id'=>$ent->id, 'type'=>2, 'status'=>1 , 'account_type_id' => 8])->first();
                                    if(isset($EntJournal)){
                                    ...
                                    if(isset($EntrefundJournal)){
                                        $erefunduser=App\User::find($EntrefundJournal->user_id);
										$EntAccTran = App\AccountTransaction::where(['journal_id'=>$EntrefundJournal['id'], 'status'=>1]);
	  									// $color = ($entAmount - $EntAccTran->sum('credit')) == 0 ? "style=font-weight:700;color:#3c8dbc" : '';
	              						// $colork = ($entKamount - $EntAccTran->sum('kcredit')) == 0 ? "style=font-weight:700;color:#3c8dbc" : '';
                                        $entranAmount = $entranAmount - $EntrefundJournal['book_amount'];
				                  	...
				                  	<td class="text-right">
                                            ...
                                            @if(isset($EntJournal['book_amount']) || isset($EntJournal['book_kamount']))
                                                @if($EntJournal['book_amount'] > 0 || $EntJournal['book_kamount'] > 0)
                                                    ...
                                                    @if($EntJournal['book_amount'] > $EntAccTran->sum('credit') || $EntJournal['book_kamount'] > $EntAccTran->sum('kcredit'))
                                                        ...
                                                        <span data-toggle="modal" data-target="#loadProjectConfirm" class="btn btn-success btn-xs AddToACcount text-right hidden-print pull-right"
                                                            ...
                                                            data-book_id="{{$ent->id}}"
                                                            data-supplier="" 
                                                            data-country="{{$ent->country_id}}">Return
                                                        </span>
                                                    @elseif($entAmount >= $EntJournal['book_amount'] || $entKamount >= $EntJournal['book_kamount'] )
                                                        <span class="btn btn-link btn-xs hidden-print pull-right">
                                                        <a target="_blank" href="{{route('getJournalReport', ['journal_entry'=>$project->project_number])}}">View/Edit</a> </span>
                                                        <span data-toggle="modal" data-target="#loadProjectConfirm" class="btn btn-success btn-xs AddToACcount text-right hidden-print pull-right"
                                                            data-type="55" 	data-kamount="{{ $entKamount }}"
                                                ...
                                                @else
                                                    ...
                                                    @if($EntJournal['book_amount'] > $EntAccTran->sum('credit') || $EntJournal['book_kamount'] > $EntAccTran->sum('kcredit'))
                                                        <a target="_blank" href="{{route('getPayable', ['journal_id'=>$EntJournal['id'] ])}}" class="btn btn-info btn-xs pull-right hidden-print" readonly><b>Pay</b></a>
                                                        <span class="btn btn-link btn-xs hidden-print pull-right">
                                ...
                                @if(isset($EntrefundJournal))
                                    <tr style="font-weight:700;color:red">
                                        <td >Refund Date : {{$EntrefundJournal->entry_date}}</td>
                                        <td  colspan="7">{{$EntrefundJournal->remark}}</td>
                                        <td  colspan="3">{{Content::money($EntrefundJournal['book_amount']) }}</td>
                                        @if(\Auth::user()->role_id == 2)
                                                    <td>{{{$erefunduser->name or ''}}}</td>                                
                                                @endif
                                    </tr> 
                                @endif 			
							@endforeach
                                            ...
                                            @if($miscService->count() > 0) 
                                                ...
                                                @foreach($miscService as $misc)
                                                    <?php 
                                                        ...
                                                        $MiscRefundJournal = App\AccountJournal::where(['business_id'=>54,'project_number'=>$project->project_number, 'book_id'=>$misc->id, 'type'=>2, 'status'=>1, 'account_type_id' => 8])->first();
                                                        ...
                                                        if(isset($MiscRefundJournal)){
                                                            $miscrefunduser=App\User::find($MiscRefundJournal->user_id);
                                                            $MiscRefundAccTran = App\AccountTransaction::where(['journal_id'=>$MiscRefundJournal['id'], 'status'=>1]);
                                                            // $color = ($miscAmount - $MiscAccTran->sum('credit')) == 0 ? "style=font-weight:700;color:#3c8dbc" : '';
                                                            // $colork = ($misckAmount - $MiscAccTran->sum('kcredit')) == 0 ? "style=font-weight:700;color:#3c8dbc" : '';
                                                            $miscAmount = $miscAmount - $MiscRefundJournal['book_amount'];
                                                        }
                                                    ...
                                                    <div class="row col-md-12" style="padding-right: 0px;">
                                                        ...
                                                        <label class="col-md-2 text-center" style="font-weight: 400; padding-right: 0px;">
                                                            ...
                                                            @endif
                                                            @if(isset($MiscJournal['book_amount']) || isset($MiscJournal['book_kamount']))
                                                                @if($MiscJournal['book_amount'] > 0 || $MiscJournal['book_kamount'] > 0)
                                                                     @if(\Auth::user()->role_id == 2 || \Auth::user()->role_id == 3)
                                                                        ...
                                                                        @if($MiscJournal['book_amount'] > $MiscAccTran->sum('credit') || $MiscJournal['book_kamount'] > $MiscAccTran->sum('kcredit'))
                                                                            ...
                                                                            <span data-toggle="modal" data-target="#loadProjectConfirm" class="btn btn-success btn-xs AddToACcount text-right hidden-print pull-right" 
                                                                                ...
                                                                                data-process_type="receive" 
                                                                                data-title="{{ $misc->servicetype->name }}" 
                                                                                data-book_id="{{$misc->id}}"
                                                                                data-supplier="" 
                                                                                data-country="{{ $tour->country_id}}">Return
                                                                            </span>
                                                                    ...
                                                                    <span class="badge badge-light pull-right">{{ $MiscAccTran->sum('credit') ? Content::money($MiscAccTran->sum('credit')) : Content::money($MiscAccTran->sum('kcredit')) }}</span>
                                                                        ...
                                                                        @if($MiscJournal['book_amount'] > $MiscAccTran->sum('credit') || $MiscJournal['book_kamount'] > $MiscAccTran->sum('kcredit'))
                                                                            <a target="_blank" href="{{route('getPayable', ['journal_id'=> $MiscJournal['id']])}}" class="btn btn-info btn-xs pull-right hidden-print  pull-right" readonly><b>Pay</b></a>
                                                    ...
                                                    @if(isset($MiscRefundJournal))
                                        <tr style="font-weight:700;color:red">
                                            <td >Refund Date : {{$MiscRefundJournal->entry_date}}</td>
                                            <td  colspan="7">{{$MiscRefundJournal->remark}}</td>
                                            <td colspan="2">{{Content::money($MiscRefundJournal['book_amount']) }}</td>
                                            @if(\Auth::user()->role_id == 2)
                                                        <td>{{{$miscrefunduser->name or ''}}}</td>                                
                                                    @endif
                                        </tr> 
                                     @endif 	
                                                @endforeach
                        ...
                        <!-- MISC End -->
                         <?php 
                            ...
                            // $grandtotal = $totalHotel + $flightAmount + $TotalGolf + $totalCruise + $restTotal + $entranAmount + $transportTotal + $guidTotal + $miscAmount;
						?>
						 <tr>
                            <td colspan="11" class="text-right"> <strong>GRAND TOTAL {{Content::currency()}} : {{ number_format(($grandtotal ), 2)}}</strong> </td>
                        </tr>
				              	...
				              	<table class="table">
					                ...
					                <tbody id="data_payment_option">
						                <tr>
						                    ...
						                    <td>
						                      	<input type="text" class="debit form-control input-sm text-right" data-type="debit" name="debit" id="debit" placeholder="00.0" >
						                    </td>
						                    <td>
						                      	<input type="text" class="credit form-control input-sm text-right" data-type="credit" name="credit" id="credit" placeholder="00.0" >
						                    </td>
						                    <td>
						                ...
						                <tr>
						                	<td colspan="7" style="border-top: none;">
						                		<label>Descriptions <span style="color: red">*</span> (Please write the descriptions for posting)</label>
						                		<textarea class="form-control" rows="4" name="payment_desc" placeholder="Type Descriptions here...!" required></textarea>
						                	</td>
						                </tr>
					                </tbody>
					            </table>
	...
	<script>
	    ...
	    function filterAccountName(){
	     ...
	     document.querySelectorAll('a[readonly], button[readonly]').forEach(element => {
